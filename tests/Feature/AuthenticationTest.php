...
test('completing onboarding with duplicate appreciation IDs fails validation', function () {
    $contributor = User::factory()->create(['username' => 'mentor_dup']);

    $response = $this->withSession([
        'onboarding_user' => [
            'google_id' => 'google-id-dup',
        ...
        'username' => 'dup_user',
        'school' => 'Notre Dame College',
        'appreciations' => [$contributor->id, $contributor->id],
    ]);

    $response->assertSessionHasErrors(['appreciations.0']);
    $this->assertGuest();
});

test('completing onboarding with more than 4 appreciations fails validation', function () {
    $contributors = User::factory()->count(5)->create();

    $response = $this->withSession([
        'onboarding_user' => [
            'google_id' => 'google-id-overflow',
        ...
        'school' => 'Notre Dame College',
        'appreciations' => $contributors->pluck('id')->all(),
    ]);

    $response->assertSessionHasErrors(['appreciations']);
    $this->assertGuest();
});
