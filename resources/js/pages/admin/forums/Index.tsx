...
export default defineComponent({
    ...
    setup(props) {
        ...
        const statusTabs = computed<{ value: StatusFilter; label: string }[]>(
            () => [
                {
                    value: 'all',
                    label: `All (${props.stats.totalPosts})`,
                },
                {
                    value: 'pending',
                    label: `Pending (${props.stats.pendingCount})`,
                },
                {
                    value: 'flagged',
                    label: `Flagged (${props.stats.flaggedCount})`,
                },
                {
                    value: 'rejected',
                    label: `Rejected (${props.stats.rejectedCount})`,
                },
                {
                    value: 'approved',
                    label: `Live (${props.stats.approvedCount})`,
                },
            ],
        );

        const statusToneClass = (status: ModerationStatus): string =>
            status === 'approved'
            ...
            <>
                ...
                <div class={adminPageClass}>
                    ...
                    <AdminPageHeader
                        ...
                        countTone="amber"
                        description="Review community questions, moderate discussions, and manage reports."
                    />

                    {/* Section switcher (same row position on Discussions / Reports / Settings) */}
                    ...
                    <div>
                        {props.posts.data.length === 0 ? (
                            ...
                            <div class={adminTableWrapClass}>
                                <table class="w-full text-left text-xs text-slate-700 dark:text-gray-300">
                                    <thead class={adminTableHeadClass}>
                                        <tr>
                                            <th class="rounded-tl-xl px-3 py-2.5 whitespace-nowrap">
                                                Discussion
                                            </th>
                                            ...
                                            <th class="rounded-tr-xl px-3 py-2.5 text-right whitespace-nowrap">
                                                Actions
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-slate-100 dark:divide-gray-800">
                                        {props.posts.data.map((post) => (
                                            <tr
                                                key={post.id}
                                                class="transition hover:bg-slate-50 dark:hover:bg-gray-800/50 last:[&>td:first-child]:rounded-bl-xl last:[&>td:last-child]:rounded-br-xl"
                                            >
